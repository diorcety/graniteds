 ...
 * @author Franck WOLFF
 */
public class SimpleServicesConfig implements ServicesConfig, ScannedItemHandler {
    ...
    public void handleClass(Class<?> clazz) {
        ...
        if (anno != null && !("".equals(anno.id()))) {
            ...
            Destination destination = new SimpleDestination(anno.id(), channelIds, props, roles, null, clazz);
    ...
    public SimpleServicesConfig(InputStream customConfigIs, Configuration configuration, boolean scan) throws IOException, SAXException {
    ...
    private void forElement(XMap element) {
        ...
        if (services != null) {
            for (XMap service : services.getAll("service")) {
                Service serv = SimpleService.forElement(service);
        ...
        if (channels != null) {
            for (XMap channel : channels.getAll("channel-definition")) {
                Channel chan = SimpleChannel.forElement(channel);
                this.channels.put(chan.getId(), chan);
            }
        }
        else {
            log.info("No channel definition found, using defaults");
            EndPoint defaultEndpoint = new SimpleEndPoint("http://{server.name}:{server.port}/{context.root}/graniteamf/amf", "flex.messaging.endpoints.AMFEndpoint");
            Channel defaultChannel = new SimpleChannel("my-graniteamf", "mx.messaging.channels.AMFChannel", defaultEndpoint, XMap.EMPTY_XMAP);
        ...
        if (factories != null) {
            for (XMap factory : factories.getAll("factory")) {
                Factory fact = SimpleFactory.forElement(factory);
